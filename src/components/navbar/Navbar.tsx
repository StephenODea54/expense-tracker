...
export const Navbar = () => {
  ...
  return(
    <header className="sticky-top-0 flex w-full drop-shadow-1 dark:drop-shadow-none bg-white dark:bg-[#23303F] z-99 shadow-lg relative">
      <div className="flex flex-grow items-center justify-between shadow-2 py-4 px-4 md:px-6 2xl:px-11">
        <div></div>
        <div className="flex items-center gap-3 sm:gap-7">
          {/* Dark Mode Toggle Here */}
          <div></div>
          <div className="relative">
            <button className="flex items-center gap-4" onClick={() => setMenuOpen(!menuOpen)}>
              {/* Name */}
              <span className="hidden lg:block text-right">
                <span className="block font-medium text-sm text-black dark:text-white">Marissa Pecchia</span>
                <span className="block text-xs">Da Wifey</span>
              </span>
              {/* Image */}
              ...
              {/* Dropdown Button */}
              <svg className="fill-current hidden sm:block" width="12" height="8" viewBox="0 0 12 8" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            {/* Menu */}
            {menuOpen ? <div className="flex flex-col shadow-default rounded-sm border border-stroke dark:border-strokedark bg-white dark:bg-boxdark w-62.5 absolute right-0 mt-4">
              <button className="font-medium text-sm lg:text-base flex items-center gap-3.5 ease-in-out duration-300 hover:text-[#3C50E0] py-4 px-6">
